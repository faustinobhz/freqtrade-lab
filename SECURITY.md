...
- O serviço mantém o usuário não-root `ftuser` da imagem oficial. Não substitua pelo UID do host: o Freqtrade é instalado no diretório pessoal de `ftuser`. Os dados persistem no volume Docker `bot_data`, mantendo as permissões da imagem. Windows nativo tem outras regras de permissão; o caminho documentado é Ubuntu/WSL.
...
- Backups do volume de dados e de `.private` devem ser privados. Não enviar para repositório, issue ou artefato de CI. `docker compose down` preserva o volume; não use `down -v`, que o remove.
